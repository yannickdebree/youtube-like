...
describe('pages (REST)', () => {
    describe('Can create a page', () => {
        it('Cannot create a page as signed out', (done) => {
            request(app.callback()).post('/pages').send().expect(401).then(({ body }) => {
        ...
        it('Cannot create a page without name', (done) => {
            request(app.callback()).post('/accounts').send({
                ...
                request(app.callback()).post('/auth').send({
                    ...
                    request(app.callback()).post('/pages').set("Authorization", body.data.accessToken).send().expect(422).then(({ body }) => {
        ...
        it('Cannot create a page with empty name', (done) => {
            request(app.callback()).post('/accounts').send({
                ...
                request(app.callback()).post('/auth').send({
                    ...
                    request(app.callback()).post('/pages').set("Authorization", body.data.accessToken).send({ name: '' }).expect(422).then(({ body }) => {
                        expect(body.message).toBe(EMPTY_NAME);
                        done();
                    });
                });
        ...
        it('Can create a page with a name', (done) => {
            request(app.callback()).post('/accounts').send({
                ...
                request(app.callback()).post('/auth').send({
                    ...
                    request(app.callback()).post('/pages').set("Authorization", body.data.accessToken).send({ name: 'My name' }).expect(201).then(({ body }) => {
